...
export DEFAULT_PASSWORD=${DEFAULT_PASSWORD:-"password1234"}
...
function give_ether_in_genesis() {
	mkdir -p /tmp/clique/give_ether_in_genesis
	...
	cat $1 | jq ".alloc += $MOREALLOCS" > /tmp/clique/give_ether_in_genesis/new_genesis.json
	rm $1
	mv /tmp/clique/give_ether_in_genesis/new_genesis.json $1
	rm -rf /tmp/clique/give_ether_in_genesis
}
...
function set_genesis_signer_address() {
	mkdir -p /tmp/clique/set_genesis_signer_address
	BAREADDRESS=`echo $2 | sed s/0x//`
	cat $1 | sed "s/DEPLOY_ACCOUNT_ADDRESS/$BAREADDRESS/" > /tmp/clique/set_genesis_signer_address/new_genesis.json
	rm $1
	mv /tmp/clique/set_genesis_signer_address/new_genesis.json $1
	rm -rf /tmp/clique/set_genesis_signer_address
}
...
function initialize_clique() {
	if [[ "$ACCOUNTS_TO_CREATE" -lt "1" ]]; then
		echo "ACCOUNTS_TO_CREATE must be at least 1 (got $ACCOUNTS_TO_CREATE)"
		exit 1
	fi

	# move the base clique genesis json to our working one
	if [ -z "$GENESIS_JSON" ]; then
	  cp /clique/genesis-default.json $CBROOT/genesis.json
	else
	...
	touch $CBROOT/.clique-initialized
}

mkdir -p $CBROOT
if [ -e $CBROOT/.clique-initialized ]; then
	echo clique appears to already be initialized, will not init
else
	echo Ephemeral or first-run of clique, will init
	initialize_clique
fi

# figure out what accounts we need to unlock, and finally run clique!
